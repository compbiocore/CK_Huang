...
###still need to remove rows that sum to 0
...
gapdh_counts <- counts_spread_ids %>% 
  dplyr::filter(hgnc_symbol == 'GAPDH')

actb_counts <- counts_spread_ids %>% 
  dplyr::filter(hgnc_symbol == 'ACTB')
...
gapdh_counts_m <- melt(gapdh_counts) %>% 
  rename('variable' = 'file_id', 'value' = 'counts')
actb_counts_m <- melt(actb_counts) %>% 
  rename('variable' = 'file_id', 'value' = 'counts')
...
gapdh_counts_meta <- dplyr::right_join(
  tcga_metadata_sample_df_cleaned, 
  gapdh_counts_m, 
  by = 'file_id') %>% 
  dplyr::select('case_id', 'file_id', 'Sample_type', 'hgnc_symbol', 'counts')

actb_counts_meta <- dplyr::right_join(
  tcga_metadata_sample_df_cleaned, 
  actb_counts_m, 
  by = 'file_id') %>% 
  dplyr::select('case_id', 'file_id', 'Sample_type', 'hgnc_symbol', 'counts')
...
gapdh_counts_meta_individ <- ggplot(gapdh_counts_meta, aes(x = file_id, y = counts, fill = Sample_type)) + geom_bar(stat='identity')

gapdh_counts_meta_individ + 
  labs(y='Counts - GAPDH') + 
  theme_classic() + 
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))

ggsave(file = 'gapdgh_fileid_plot.pdf')
```

```{r}
#make plots of individuals from raw data

actb_counts_meta_individ <- ggplot(actb_counts_meta, aes(x = file_id, y = counts, fill = Sample_type)) + geom_bar(stat='identity')

actb_counts_meta_individ + 
  labs(y='Counts - ACTB') + 
  theme_classic() + 
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))

ggsave(file = 'actb_fileid_plot.pdf')
